## 👋 Hi, I am Swastik

---

## 🧠 About

- ⚙️ Building backend systems and data pipelines for AI applications  
- 🤖 RAG chatbots
- 📚 Currently learning agentic AI systems using Claude Code & Codex  

---

## 🛠️ Tech Stack

| 🏷️ Area        | ⚡ Tools |
|---------------|--------|
| 🧩 **Backend** | FastAPI, Spring Boot, Actix|
| 🗄️ **Data**    | PostgreSQL, Redis |
| 🧠 **GenAI**   | Qdrant, Pinecone |
| ☁️ **Infra**   | Docker |

---

## 🌐 Connect

- 🔗 [LinkedIn](https://www.linkedin.com/in/swastik-nandy)  
- 📧 [Email](mailto:swastik.nandy@outlook.com)  
- 💻 [GitHub](https://github.com/swastik-nandy)
